...
MasterJs={		    		    
    	...
    	//------------- functions to change background from admin panel --------------//  
            changeBackgroundCss:function()
            {
            	var colorCode = "#"+document.getElementById("backColor").value;
            	$(".body_background").css("background-color", colorCode);
            },
            
            addHeader:function(id, userName)
            {
            	
            	var oContent = document.getElementById(id);
            	oContent.innerHTML = '<div id="header"><div class="logo"> <img src="images/logo.png" alt="" /></div>'
            					+'<div class="rightPart"><nav><ul><li> <a >About My Agnitio</a> </li><li> <a href="mailto:info@agnitio-technologies.com" style="text-decoration: none;">Support</a></li><li> <a href="logout">Logout </a></li></ul></nav>'
            					+'<div class="header-part"><div class="wlcome-user"><div class="icon" id="iconUser"><i class="user"></i></div>Welcome '+userName+' </div>'
            					+'<ul class="nav-top"><li class="search-icon"><a href="#"></a></li><li class="location"><a href="#"></a></li><li class="calendar"> <a href="#"></a></li>'
            					+'<li class="map"> <a href="#"></a></li><li class="bag"> <a href="#"></a></li><li class="print"> <a href="#"></a></li><li class="setting"> <a href="#"></a></li>'
            					+'</ul><div class="input-group search " id="searchBox" style="display: none;"><input type="text" class="form-control search-input" placeholder="Search">'
            					+'<span class="input-group-addon" id="btnSearchClose"><button type="submit"><span class="glyphicon glyphicon-search"></span> </button></span> </div></div></div></div>';
            },    
            
            addFooter:function(id)
            {
            	var oContent = document.getElementById(id);
            	oContent.innerHTML = '<footer><p>Copyright &#64; 2014 Agnitio Technologies. All Rights Reserved.</p></footer>';
            },
            
            addTaskbar:function(rows, arrString, id, linkPages)
            {
            	var totalRows = rows;
                var cellsInRow = 1;
                var div1 = document.getElementById(id);
                $(div1).addClass("taskbar");
                var tbl = document.createElement("div");
                tbl.className = 'taskbar-div';
                for (var r = 0; r < totalRows; r++) {
                    var row = document.createElement("div");
                    row.className = 'taskbar-row';
                    for (var c = 0; c < cellsInRow; c++) {
                        var cell = document.createElement("div");
                        cell.className = 'taskbar-cell';
                        cell.setAttribute('id',linkPages[r]);
                        cell.setAttribute('onClick','MasterJs.changeUrl(this.id)');
                        var cellText = document.createTextNode(arrString[r]);
            ...
            addMenu:function(cells, arrString, id, linkPages)
            {
            	var totalRows = 1;
            	var cellsInRow = cells;
            	var div1 = document.getElementById(id);
            	$(div1).addClass("menubar");
            	var tbl = document.createElement("table");
            	tbl.className = 'menubar-div';
            	for (var r = 0; r < totalRows; r++) {
            		var row = document.createElement("tr");
            		row.className = 'menubar-row';
            		for (var c = 0; c < cellsInRow; c++) {
            			var cell = document.createElement("td");
            			cell.className = 'menubar-cell';
            			var cellWidth = 100/cells;
            			cell.style.width = cellWidth + "%";
            			cell.setAttribute('id',linkPages[c]);
            			cell.setAttribute('onClick','MasterJs.changeUrl(this.id)');
            			var cellText = document.createTextNode(arrString[c]);
            			cell.appendChild(cellText);
            			row.appendChild(cell);
            }           
            ...
            changeUrl:function(clickid)
            {
            	 document.getElementById('independentPage').src = clickid;
            	 //cell.setAttribute('empProfile.taskmenu(clickid)');
            	 empProfile.taskmenu(clickid);
            	 //adminempProfile.taskmenu(clickid);
            },
            
            addBirthdayWidget:function()
            {
            	var sliderOptions=
                {
                	sliderId: "slider",
